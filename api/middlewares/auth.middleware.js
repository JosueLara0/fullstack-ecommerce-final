...
exports.protectSessionOwner = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  const { currentUser } = req;

  // Validate currentUser id with the given user id
...
exports.protectProductOwner = catchAsync(async (req, res, next) => {
  ...
  if (product.userId !== currentUser.id) {
    return next(new AppError("You don not own this product", 401));
  }
